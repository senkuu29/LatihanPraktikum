...
// Ganti nama pasien berdasarkan ID-nya
void updateNama(int idCari, char namaBaru[]) {
    struct Node *bantu = head;
    while (bantu != NULL) {
        if (bantu->id == idCari) {
            strcpy(bantu->nama, namaBaru); // Timpa nama lama dengan yang baru
            printf("Data pasien dengan id %d berhasil diperbarui\n", idCari);
            return;
        }
        bantu = bantu->next;
    }
...
// Hapus pasien tertentu berdasarkan namanya
void hapusTengah(char namaCari[]) {
    if (head == NULL) return;  // Kalau kosong ya nggak ada yang bisa dihapus

    struct Node *bantu = head;
    struct Node *sebelum = NULL;

    // untuk mencari nama orang  
    while (bantu != NULL && strcmp(bantu->nama, namaCari) != 0) {
        sebelum = bantu;        // Simpan posisi sebelumnya
        bantu = bantu->next;    // Maju terus sampai ketemu atau habis
    }

    // Kalau namanya ketemu di list
    if (bantu != NULL) {
        // Kalau ternyata yang dihapus itu orang pertama
        if (sebelum == NULL) {
            head = head->next;
        } else {
            // "Lompatin" node yang dihapus: sambungin orang sebelumnya ke orang sesudahnya
